...
require_once __DIR__ . '/../bo/TblCommunityBO.php';
...
$log = Logger::getLogger('CommunityDAO(Outside)');
  ...
  function fetchAll()
	{
      $this->log->debug('fetchAll() - ENTER');
	    ...
	    $sql = "SELECT DISTINCT NAME from TblCommunity";
	    ...
	    try
	    {
	        ...
	        $communityNameArray = array();

	        while($row = $query->fetch())
	        {
	            array_push($communityNameArray, $row['NAME']);
	        }
      ...
      $this->log->debug('communityNameArray contains : ');
      $this->log->debug('---------------------------');
      foreach ($communityNameArray as $value) {
