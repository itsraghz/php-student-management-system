...
class UserRoleDAO extends BaseDAO
{
  ...
  function isUserNotAStudent($UserId) {
    $this->log->debug('isUserNotAStudent() - ENTRY');
    $this->log->info('UserId: ' . $UserId);
    ...
    $this->log->info('isUserAStudent : ' . $isUserAStudent);
    $this->log->info('isUserNotAStudent : ' . ($isUserAStudent==1 ? 0 : 1));
    $this->log->debug('isUserNotAStudent() - EXIT');

    return ($isUserAStudent==1 ? 0 : 1);
  }
  ...
  function isUserAnAdmin($UserId) {
    $this->log->debug('isUserAnAdmin() - ENTRY');
    $this->log->info('UserId: ' . $UserId);
    ...
    if(!empty($roleNamesArray) || count($roleNamesArray)>=1) {
      if (in_array("Admin", $roleNamesArray)) {
        $isUserAnAdmin=1;
      }
    }

    $this->log->info('isUserAnAdmin : ' . $isUserAnAdmin);
    $this->log->debug('isUserAnAdmin() - EXIT');

    return $isUserAnAdmin;
  }

  function isUserAnAccounts($UserId) {
    $this->log->debug('isUserAnAdmin() - ENTRY');
    $this->log->info('UserId: ' . $UserId);
    ...
    if(!empty($roleNamesArray) || count($roleNamesArray)>=1) {
      if (in_array("Accounts", $roleNamesArray)) {
        $isUserAnAccounts=1;
      }
    }

    $this->log->info('isUserAnAccounts : ' . $isUserAnAccounts);
    $this->log->debug('isUserAnAccounts() - EXIT');

    return $isUserAnAccounts;
  }
}
